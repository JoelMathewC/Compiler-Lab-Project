...
program : Declarations START Slist END ENDSTMT	{ 	
	...
	| Declarations START END ENDSTMT		{ 
					  			exit(0);
					  		}
	| START END ENDSTMT				{ 
					  			exit(0);
...
IdentifierDecl: ID					{
		...
		| STAR ID				{
								...
								shape[1] = 0; 
								validateShape(shape);
								$$ = declIdNode($<string>2,-1,shape); //dimensions for pointers is set to -1 at the start
							}
		;
...
AssgStmt: Identifier ASSIGN expr ENDSTMT		{$$ = makeOperatorNode(assign,$1,$3);}
	| Identifier ASSIGN ADDR Identifier ENDSTMT		{$$ = makePtrIdNode($1, st, $4);}
	;
...
Identifier : ID				{
	...
	| STAR ID				{
							...
							index2 = NULL; 
							$$ = makeIdNode($<string>2,st,index1,index2);	
						}
	;
